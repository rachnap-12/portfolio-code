...
export const getIsValidOnNameTag = () => {
  var isFirstLetterUppercase = /^[A-Z]/.test(nameTagOption);
  if((nameTagOption.length <= 8) &&  isFirstLetterUppercase) {
    return true;
  }
};
...
export const getHasUppercaseLetters = () => {
  if (/[A-Z]/.test(getHasUppercaseLetters)){
    return true;
  }
};
...
export const convertPascalCaseToSnakeCase = () => {
  var s1 = pascalCaseVariableName.split(/(?=[A-Z])/).join('_').toLocaleLowerCase();
  return s1;
};
